...
* Internal refactor: store extension operators added via `open type` in a name-indexed `NameMultiMap<MethInfo>` (`eOpenedTypeOperators`) instead of a flat list. `SelectExtensionMethInfosForTrait` now does a direct bucket lookup for the trait name. Source declaration order of operators within a single `open type` is preserved (see `List.foldBack` invariant comment in `AddStaticContentOfTypeToNameEnv`). Also dedups a hand-rolled `Dictionary<string, MethInfo list>` grouping in `CheckEntityDefn` into the same `NameMultiMap.initBy` convention. No public API change; no user-visible behavior change.
